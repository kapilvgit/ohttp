...
extern {
    fn get_attestation_token(app_data: *const u8, pcr_sel: u32, jwt: *mut u8,  jwt_len: *mut size_t, endpoint_url: *const char) -> c_int;
}

pub fn attest(data: &[u8], pcrs: u32) -> Option<Vec<u8>> {
    unsafe {
        let url_ptr: *const char = MAA_ENDPOINT_URL.as_ptr() as *const char;
